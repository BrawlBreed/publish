...
const { orderMessage, generateSubject } = require("../utils/constants");
...
exports.newOrder = asyncWrapper(async (req, res, next) => {
  ...
  const order = await orderModel.create({  
    shippingInfo,
    orderItems,
    paymentInfo : { id: "123", status: "success" },
    itemsPrice,
    taxPrice,
    shippingPrice,
    totalPrice,
  ...
  const html = orderMessage(order)

  await sendEmail({
    email: req.user.email,
    subject: `${generateSubject(order.orderStatus)}, ${req.user.name}`,
    html
  })
...
exports.updateOrder = asyncWrapper(async (req, res, next) => {
  ...
  await sendEmail({
    ...
    subject: `${generateSubject(order.orderStatus)}, ${order.shippingInfo.firstName}`,
    html
  })
