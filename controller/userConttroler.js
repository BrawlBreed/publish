...
const {forgotPasswordMessage} = require("../utils/constants");
...
exports.forgotPassword = asyncWrapper(async (req, res, next) => {
  ...
  const html = forgotPasswordMessage(resetPasswordUrl)
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      message: `Имейлът за смяна на парола е изпратен успешно!`,
    });
  } catch (error) {
...
exports.resetPassword = asyncWrapper(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ErrorHandler(
        "Паролата за нулиране е невалидна или е изтекла! Опитайте отново.",
        400
      )
    );
  }
...
exports.updateProfile = asyncWrapper(async (req, res, next) => {
  ...
  if (req.body.avatar !== "" || !req.body.avatar) {
    ...
    const imageId = user?.avatar?.public_id || newUserData.avatar?.public_id;
  ...
  const user = await userModel.findByIdAndUpdate(req.body.user._id, newUserData, {
    new: true,
    runValidators: true,
